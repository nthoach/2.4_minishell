...
/*   Updated: 2024/04/23 17:43:14 by nthoach          ###   ########.fr       */
/*                                                                            */
/******************************************************************************/
...
int	executor(t_utils *utils)
{
	...
	fd_in = STDIN_FILENO;
	curr_cmd = utils->cmds; //first command
	while (curr_cmd)
	{
		if (curr_cmd->next)
			if (pipe(end) < 0)
...
int	prepare_executor(t_utils *utils)
{
	...
	if (utils->pipes == 0)
		single_cmd(utils->cmds, utils); //execution single cmd
	else
	{
		utils->pid = ft_calloc(utils->pipes + 2, sizeof(int));
		if (!utils->pid)
			return (ft_error(1));
		if (executor(utils)) //execution here
			return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
